...
type Color =
  ...
  | 'pink'
  | 'rose';
type Palette = Record<Color, Record<Shade, string>>;

const SHADE_STEP = 100;

const colors: [Color, [string, string, string, string, string, string, string, string, string, string]][] = [
  [
    'gray',
...
export const palette: Palette = colors.reduce(
  (acc, [key, values]) => ({
    ...acc,
    [key]: Object.fromEntries(
      new Array(values.length)
        .fill(SHADE_STEP)
        .map((step, index) => [index > 0 ? index * step : step / 2, values[index]])
    ),
  }),
  {} as Palette
);
